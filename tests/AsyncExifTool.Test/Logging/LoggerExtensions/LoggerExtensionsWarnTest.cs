namespace CoenM.ExifToolLibTest.Logging.LoggerExtensions
{
    ...
    public class LoggerExtensionsWarnTest
    {
        ...
        [Fact]
        public void Warn_ShouldAlwaysPassMessageToLogger()
        {
            // arrange

            // act
            Sut.Warn(_logger, "test message");

            // assert
            A.CallTo(() => _logger.IsEnabled(A<LogLevel>._)).MustNotHaveHappened();
            A.CallTo(() => _logger.Log(A<LogEntry>._)).MustHaveHappenedOnceExactly();
            _logEntries.Should().BeEquivalentTo(new LogEntry(LogLevel.Warn, "test message"));
        }
        ...
        public void Warn_WithLambdaExpression_ShouldPassMessageToLogger_WhenLogLevelIsEnabled()
        {
            ...
            A.CallTo(() => _logger.IsEnabled(LogLevel.Warn)).Returns(true);

            // act
            Sut.Warn(_logger, TestMessageGenerator);

            // assert
            A.CallTo(() => _logger.IsEnabled(LogLevel.Warn)).MustHaveHappenedOnceExactly()
             .Then(A.CallTo(() => _logger.Log(A<LogEntry>._)).MustHaveHappenedOnceExactly());
            _logEntries.Should().BeEquivalentTo(new LogEntry(LogLevel.Warn, "Test message generated"));
        ...
        public void Info_ShouldReturn_WhenMessageGeneratorIsNull()
        {
            ...
            // act
            Sut.Warn(_logger, (Func<string>)null);
